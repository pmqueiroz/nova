; Custom NSIS template for Nova.
; Based on cargo-packager default template, with PATH support added.
;
; Adds "$INSTDIR" to PATH on install and removes it on uninstall.
; Uses:
; - per-user PATH when INSTALLMODE is currentUser
; - per-machine PATH when INSTALLMODE is perMachine
; - for INSTALLMODE=both, respects the selected mode via SetShellVarContext / SHCTX.
...
Section Install
  ...
  ; Add Nova to PATH so `nova` works in cmd/pwsh.
  Call AddNovaToPath
